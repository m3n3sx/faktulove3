...
# Import enhanced authentication views
from faktury.views_modules.enhanced_auth_views import (
...
# Import company dashboard views
from faktury.views_modules.company_dashboard import (
    company_dashboard,
    company_info,
    company_settings,
    company_api_status
...
from faktury.views_modules.superadmin_views import (
...
def pobierz_dane_z_gus(request):
    """
    Enhanced function to fetch company data from GUS API
    Uses the new GUS service with proper session management
    """
    from faktury.services.gus_service import gus_service
    
    nip = request.GET.get('nip', '').strip()
    regon = request.GET.get('regon', '').strip()
    
    if not nip and not regon:
        return JsonResponse({'error': 'Numer NIP lub REGON jest wymagany.'}, status=400)
    
    try:
        if nip:
            result = gus_service.search_by_nip(nip)
        else:
            result = gus_service.search_by_regon(regon)
        ...
        if result.get('success'):
            ...
            return JsonResponse({
                ...
                'numer_domu': company_data.get('numer_domu', ''),
                'numer_mieszkania': company_data.get('numer_mieszkania', ''),
                ...
                'kraj': company_data.get('kraj', 'Polska'),
                # Additional data for potential future use
                'status': company_data.get('status', ''),
                'nazwa_skrocona': company_data.get('nazwa_skrocona', ''),
                'wojewodztwo': company_data.get('wojewodztwo', ''),
                'powiat': company_data.get('powiat', ''),
                'gmina': company_data.get('gmina', ''),
            })
        else:
            error_message = result.get('error', 'Nieznany błąd podczas pobierania danych z GUS')
            return JsonResponse({'error': error_message}, status=404)
            
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Unexpected error in pobierz_dane_z_gus: {e}")
        return JsonResponse({'error': 'Wystąpił nieoczekiwany błąd. Spróbuj ponownie.'}, status=500)

...
const STATUS_CONFIG: Record<ComplianceStatus, {
  ...
  compliant: {
    ...
    bgClasses: 'bg-success-50 border-success-200',
    icon: (
  ...
  warning: {
    ...
    bgClasses: 'bg-warning-50 border-warning-200',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
  ...
  'non-compliant': {
    ...
    bgClasses: 'bg-error-50 border-error-200',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
  ...
  pending: {
    ...
    bgClasses: 'bg-neutral-50 border-neutral-200',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
...
export const ComplianceIndicator: React.FC<ComplianceIndicatorProps> = ({
  ...
  const getOverallStatus = (): ComplianceStatus => {
    if (rules.some(rule => rule.status === 'non-compliant')) return 'non-compliant';
      ...
      <div className="flex items-center space-x-4 text-xs text-neutral-600 mb-2">
        ...
        {statusCounts['non-compliant'] > 0 && (
          <span className="flex items-center">
            ...
            {statusCounts['non-compliant']} niezgodne
          </span>
        )}
        ...
        <div className="space-y-2">
          {rules.map((rule) => {
            ...
            return (
              <div
                key={rule.id}
                className="flex items-start space-x-2 p-2 bg-white bg-opacity-50 rounded"

  ...
  render: (args) => (
    <Form {...args}>
      ...
      <FormField name="message" label="Wiadomość" rules={{ minLength: 10, maxLength: 500 }}>
        <textarea 
          className="w-full px-3 py-2 border border-neutral-300 rounded-md-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          rows={4}
  ...
  render: (args) => (
    <Form {...args}>
      ...
      <FormField name="vatRate" label="Stawka VAT" rules={{ required: true }}>
        <Select
          options={[
            { value: '', label: 'Wybierz stawkę VAT' },
            { value: '23', label: '23%' },
            { value: '8', label: '8%' },
            { value: '5', label: '5%' },
            { value: '0', label: '0%' },
      ...
      <FormField 
        ...
        rules={{ 
          ...
          custom: (value) => {
            ...
            // This is just a demo - normally you'd compare with another field
            return value.length < 6 ? 'Hasło musi mieć co najmniej 6 znaków' : null;
          }
        }}
      >
    ...
    return (
      <Form {...args} onSubmit={handleSubmit}>
        ...
        <FormField name="description" label="Opis" rules={{ required: true, minLength: 10 }}>
          <textarea 
            className="w-full px-3 py-2 border border-neutral-300 rounded-md-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:bg-neutral-100 disabled:cursor-not-allowed"
            rows={4}

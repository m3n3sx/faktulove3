    ...
    if (pobierzDaneGUSButton && nipInput) {
        pobierzDaneGUSButton.addEventListener("click", function() {
            ...
            if (nipClean.length !== 10) {
                displayErrorMessage('NIP musi mieć dokładnie 10 cyfr');
                return;
            }
            
            // Pokaż informację o ładowaniu
            pobierzDaneGUSButton.disabled = true;
            pobierzDaneGUSButton.textContent = 'Pobieranie...';
            
            // Przywróć przycisk po zakończeniu (w then/catch)
            ...
            fetch(`/pobierz_dane_z_gus/?nip=${nip}`)
                ...
                .then(data => {
                    if (data.success && data.data) {
                        ...
                        console.log('Dane z GUS pobrane pomyślnie:', data.data);
                        
                        // Pokaż komunikat sukcesu (ale nie jako błąd)
                        const successMsg = document.createElement('div');
                        successMsg.className = 'alert alert-success alert-dismissible fade show mt-2';
                        successMsg.innerHTML = `
                            Dane z GUS pobrane pomyślnie!
                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        `;
                        nipInput.parentNode.appendChild(successMsg);
                        ...
                        displayErrorMessage(data.error || 'Nie udało się pobrać danych z GUS');
                    }
                })
                .catch(error => {
                    console.error("Błąd:", error);
                    displayErrorMessage(`Błąd podczas pobierania danych z GUS: ${error.message}`);
                })
                .finally(() => {
                    // Przywróć przycisk
                    pobierzDaneGUSButton.disabled = false;
                    pobierzDaneGUSButton.textContent = 'Pobierz z GUS';
                });
        });
    }
    ...
    function updateTotals() {
        try {
            ...
            const elements = {
                ...
                '#kwota-do-zaplaty': sumaBrutto.toFixed(2) + ' PLN'
            };
